import { createBooking } from '../lib/supabase.js';
...
export function renderBooking(id, params) {
  ...
  const listing = { name: bookingName || id, price: total, coverImage: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=400&q=80', type };
          ...
          <div>
            ...
            ${DEMO_MODE ? `
              <div style="background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.3);border-radius:var(--radius);padding:20px;margin-bottom:28px">
                ...
                <div style="font-size:0.85rem;color:var(--text-muted)">Payment is running in <strong>demo mode</strong>. Add your Razorpay key to <code style="background:var(--glass);padding:2px 6px;border-radius:4px">VITE_RAZORPAY_KEY</code> in <code>.env</code>.</div>
              </div>
            ` : ''}
            ...
            <div class="grid-2">
              <div class="form-group">
                ...
                <input type="text" class="form-input" id="pay-name" placeholder="Your full name" value="${user?.full_name || user?.fullName || ''}" />
              </div>
              <div class="form-group">
                <label class="form-label">Email</label>
                <input type="email" class="form-input" id="pay-email" placeholder="email@example.com" value="${user?.email || ''}" />
              </div>
              <div class="form-group">
                <label class="form-label">Phone</label>
                <input type="tel" class="form-input" id="pay-phone" placeholder="+91 98765 43210" value="${user?.phone || ''}" />
              </div>
              <div class="form-group">
            ...
            <button class="btn btn-primary btn-lg w-full" id="pay-btn" style="justify-content:center;font-size:1.1rem" ${!isLoggedIn() ? 'disabled style="opacity:0.5;cursor:not-allowed;justify-content:center;font-size:1.1rem"' : ''}>
              <span id="pay-label">${DEMO_MODE ? '🎭 Complete Demo Booking' : `🔒 Pay ₹${total.toLocaleString()} with Razorpay`}</span>
...
export function initBooking(id, params) {
  ...
  btn?.addEventListener('click', async () => {
    ...
    const bookingData = {
      listingId:   id,
      listingName: bookingName,
      listingType: type,
      checkin, checkout, guests, total,
    ...
    const options = {
      ...
      handler: async function(response) {
        setLoading(true);
        try {
          const booking = await createBooking({ ...bookingData, razorpayPaymentId: response.razorpay_payment_id });
          showToast('Payment Successful! 🎉', `Ref: ${booking.id}`);
          setTimeout(() => window.router.navigate('/booking-confirmed'), 800);
        } catch (e) {
          showToast(e.message || 'Booking save failed', '', 'error');
        } finally {
          setLoading(false);
        }
      },
